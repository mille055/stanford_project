...
def image_to_tensor(filepath, transforms = data_transforms, device=device):
    '''
    Gets a tensor from an image filepath after undergoing the appropriate transform. 
    Input: 
        filepath(path): the path to the dicom file
        transforms(Transforms): the dictionary of transforms for train/val/test to get the input vector for the model
        device(cpu or gpu)

    Output:
        input_tensor(Torch tensor): appropriate for input to the model
    '''
...
def pixel_inference(model, filelist, classes=classes, device=device):
    '''
    Gets the predictions and probablities for a given set of files 
    Input: 
        model(model): the trained model being assessed
        filelist(list[str]): list of files to evaluate, often a single image
